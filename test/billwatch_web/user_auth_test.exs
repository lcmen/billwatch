defmodule BillwatchWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, BillwatchWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "clears session when user does not match when re-authenticating", %{conn: conn, user: user} do
      ...
      refute get_session(conn, :to_be_removed)
      assert token = get_session(conn, :user_token)
      assert {returned_user, _} = Accounts.get_user_by_session_token(token)
  ...
  describe "fetch_current_scope_for_user/2" do
    ...
    test "reissues a new token after a few days and refreshes cookie", %{conn: conn, user: user} do
      ...
      update_user_token_authenticated_at(token, -10, :day)
  ...
  describe "require_authenticated_user/2" do
    ...
    test "redirects unconfirmed user to home", %{conn: conn} do
      unconfirmed_user = unconfirmed_user_fixture()

      conn =
        conn
        |> assign(:current_scope, Scope.for_user(unconfirmed_user))
        |> fetch_flash()
        |> UserAuth.require_authenticated_user([])
      ...
      assert Phoenix.Flash.get(conn.assigns.flash, :error) ==
               "You must confirm your email address to access this page."
    end

    test "allows confirmed user through", %{conn: conn, user: user} do
      # user fixture creates confirmed users by default
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> UserAuth.require_authenticated_user([])
    ...
    test "does not store return_to path for unconfirmed user", %{conn: conn} do
      unconfirmed_user = unconfirmed_user_fixture()
      ...
      assert halted_conn.halted
      # Should NOT store return_to for unconfirmed users
      refute get_session(halted_conn, :user_return_to)
    end
  end
end
